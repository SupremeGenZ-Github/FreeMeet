...
export async function meetingCapture(getStreams) {
 ...
 const audio=new Audio();await audio.resume();const dest=audio.createMediaStreamDestination(),sources=new Map();
 ...
 if(v&&!v.classList.contains('hidden-video')&&v.readyState>=2){const s=Math.min((w-8)/v.videoWidth,(h-32)/v.videoHeight);ctx.drawImage(v,x+(w-v.videoWidth*s)/2,y+(h-32-v.videoHeight*s)/2,v.videoWidth*s,v.videoHeight*s);}else{ctx.fillStyle='#16243d';ctx.fillRect(x+4,y+4,w-8,h-8);}
 ...
 draw();const stream=c.captureStream(10);dest.stream.getAudioTracks().forEach(t=>stream.addTrack(t));
 return {stream,stop(){stopped=true;clearTimeout(timer);sources.forEach(s=>s.disconnect());stream.getTracks().forEach(t=>t.stop());audio.close().catch(()=>{});}};
}
