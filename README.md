...
- **Local recording:** records the selected screen or tab using browser-supported recording features.
...
```json
{"ok":true,"version":"2.0.0","storage":"postgresql"}
```
...
Shared notes use **latest edit wins**. Take turns typing to avoid overwriting simultaneous changes.

Local recording captures the surface and audio offered by the browser, not an automatic mix of every participant. It may omit the recorder's microphone. Recording uses memory and stops after 20 minutes. Obtain participant consent before recording.
...
Validation recorded for the SkyMeet update on 23 September 2026:

- Production build passed.
- All 10 Node/server tests passed.
- Three targeted Chromium tests passed: mobile landing layout, eight theme variants with saved preferences, and two-participant collaboration.
- Collaboration coverage included guest image upload, undo/redo synchronization, sticky-note text, templates, chat and notes in both directions, JSON export, inspector visibility, and mobile canvas layout.
- The full camera test could not establish a remote stream in the test environment. Offer/answer signaling completed, but Chromium produced no ICE candidates. Live audio/video still requires deployment testing on real devices and networks. The calling implementation was unchanged.
...
| Notes overwrite another participant's changes | The current notes model is latest-edit-wins; take turns typing |
